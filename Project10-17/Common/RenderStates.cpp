...
ComPtr<ID3D11SamplerState> RenderStates::SSAnisotripicWrap = nullptr;
...
void RenderStates::InitAll(ID3D11Device* device)
{
	...
	sampDesc.MinLOD = 0;
	sampDesc.MaxLOD = D3D11_FLOAT32_MAX;
	HR(device->CreateSamplerState(&sampDesc, SSAnisotripicWrap.GetAddressOf()));
	...
	//透明混合模式
	// Color = SrcAlpha * srcColor + (1-SrcAlpha) * DescColor
	// Alpha = SrcAlpha
	...
	//不写入深度信息
	//无论是正面还是背面，原来指定区域的模板值都会被写入StencilRef
